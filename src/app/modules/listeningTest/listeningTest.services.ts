...
// live per-user fallback in assignListeningTest generates its own on
...
const generateOne = async (difficulty: Difficulty) => {
  try {
    ...
    throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, `Failed to generate listening test: ${message}`);
  }
};
