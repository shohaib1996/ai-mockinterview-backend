...
// Standard published approximate Listening raw-score-to-band table (same approximation caveat).
const LISTENING_BAND_TABLE: { min: number; band: number }[] = [
  { min: 39, band: 9 },
  { min: 37, band: 8.5 },
  { min: 35, band: 8 },
  { min: 32, band: 7.5 },
...
export const rawScoreToListeningBand = (rawScore: number, totalQuestions: number): number => {
  const scaled = totalQuestions === 40 ? rawScore : Math.round((rawScore / totalQuestions) * 40);
  const entry = LISTENING_BAND_TABLE.find((e) => scaled >= e.min);
  return entry?.band ?? 0;
};
